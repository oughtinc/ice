...
Here are the papers above, and excerpts from those papers:
...
Ideal answer to the question "How does l-theanine affect anxiety?" based on the papers above using references: Overall, the evidence points to l-theanine being effective for anxiety. Two of the three papers indicate that l-theanine reduces anxiety in animals (Ogawa et al., 2017) and humans (Kimura et al., 2007), while only one study indicates that l-theanine may not be effective for anxiety (Lu et al., 2004).""",
...
RANKING_PROMPT = """Below we have 4 summaries about "{question}".

{summary_str}

A good summary be:
- Supported ENTIRELY by the evidence
- Cite ALL the papers above(in the form (Author, Year) E.g Ogawa et al. (2017))
- Helpful at answering the question "{question}"

Let's think step by step about which summary is the best.

For each summary, please score the summary on a scale of 1-5, where 5 is the best and 1 is the worst. Give detailed reasoning for your score.

A summary should get a 5 if:
- Supported ENTIRELY by the evidence
- Cite ALL of the 4 papers above(in the form (Author, Year) E.g Ogawa et al. (2017))
- Helpful at answering the question "{question}"
- Is is sufficiently detailed but not too long

A summary should get a 4 if:
- Supported ENTIRELY by the evidence
- Cite ALL of the 4 papers above(in the form (Author, Year) E.g Ogawa et al. (2017))
- Somewhat helpful at answering the question "{question}"
- It is sufficiently detailed but could be shorter

A summary should get a 3 if:
...
- Cite SOME of the 4 papers above(in the form (Author, Year) E.g Ogawa et al. (2017))
- Vaguely helpful at answering the question "{question}"
- It contains some details

A summary should get a 1 if:
- Supported NOT AT ALL by the evidence
- Cite NONE of the 4 papers above(in the form (Author, Year) E.g Ogawa et al. (2017))
...
After you have scored summary 5, write "END" to indicate that you are done.
...
async def paragraph_synthesis(
    question: str = "How many adolescents use social media every day?",
):
    ...
    for choice in choices:
        text = choice["text"]
        print(text)
        scores_strings = re.findall(r"Score: \d", text)
        all_scores.append(list(parse_scores(scores_strings)))
