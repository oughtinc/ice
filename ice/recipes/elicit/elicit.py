import json
import os

from functools import reduce

import httpx

from tenacity import retry
from tenacity import stop_after_attempt
from tenacity import wait_random_exponential

from ice.cache import diskcache
from ice.recipe import recipe

ELICIT_BACKEND_ENDPOINT = "https://inference.elicit.org/elicit-black/lit-review"

# ELICIT_AUTH_TOKEN can be found by checking idToken in cookies for elicit.org
ELICIT_AUTH_TOKEN = os.getenv("ELICIT_AUTH_TOKEN")


def make_request_body(query: str, stop: int = 4) -> dict:
    return dict(
        query=query,
        start=0,
        stop=stop,
        # filters=dict(
        #     has_pdf=True,
        #     study_types=["RCT"] if rct_only else [],
        # ),
    )


def _merge(recurse, path: list, base: dict, nxt: dict) -> dict:
    for k, v in nxt.items():
        if k not in base:
            base[k] = v
        else:
            base[k] = recurse(recurse, path + [k], base[k], v)
    return base


def deep_merge(base, nxt):
    """
    Performs a *limited* deep merge of nxt into base.
    Type differences are overriden by nxt.
    Lists are extended, but elements are not changed or recursed into.
    Sets are unioned but not recursed into.
    """

    def merge_strategy(
        merge_strategy,
        path: list,
        base,
        nxt,
    ):
        if not (isinstance(base, type(nxt)) or isinstance(nxt, type(base))):
            return nxt
        elif isinstance(nxt, dict):
            return _merge(merge_strategy, path, base, nxt)
        elif isinstance(nxt, list) or isinstance(nxt, tuple):
            return base + nxt
        elif isinstance(nxt, set):
            return base | nxt
        return nxt

    return merge_strategy(merge_strategy, [], base, nxt)


@diskcache()
@retry(stop=stop_after_attempt(2), wait=wait_random_exponential(2))
def merged_elicit_response(query: str):
    assert ELICIT_AUTH_TOKEN is not None, "ELICIT_AUTH_TOKEN is not set"
    headers = {
        "Content-Type": "application/json",
        "Authorization": ELICIT_AUTH_TOKEN,
    }
    chunks = []
    with httpx.stream(
        "POST",
        ELICIT_BACKEND_ENDPOINT,
        json=make_request_body(query),
        headers=headers,
        timeout=40,
    ) as r:
        for chunk in r.iter_text():
            print(chunk)
            chunks.append(chunk)
    joined = "".join(chunks)
    split_on_newlines = joined.split("\n")
    as_json = [json.loads(line) for line in split_on_newlines if line]
    merged = reduce(deep_merge, as_json)
    return merged


async def elicit(question: str = "What is the effect of creatine on cognition?"):
    # TODO: Support more query arguments
    response = merged_elicit_response(question)
    return response


recipe.main(elicit)
