from ice.recipes.elicit.synthesize_ft import n_tokens, synthesize_charlie
...
def _create_in_app_prompt(query: str, titles: list[str], citations: list[str], abstracts: list[str]) -> str:
    # Use with text-davinci-002
    # With Andreas' tweaks
    papers_str = "\n\n".join(
        [
            PAPER_FORMAT.format(
    ...
    while (
        n_tokens(prompt) > MAX_TOKENS - MIN_COMPLETION_TOKENS and number_of_shots > 0
    ):
        number_of_shots -= 1
...
async def synthesize_in_app_from_df(
    question,
    papers,
    **kwargs
):
    return await synthesize_charlie(
        question,
        [Abstract(
            title=paper["title"],
