...
EXAMPLES: list[dict[str, str | StopSentinel]] = [
    dict(
...
PREFIX = "Instructions: Enrich each Passage with the Context."

DECONTEXT_EXAMPLE = """Context: {context}
Passage: {passage}
Rewrite: {rewrite}
""".strip()

# TODO: integrate question focus


def decontext_prompt(context: str, passage: str) -> str:
    examples = format_multi(
        DECONTEXT_EXAMPLE,
        EXAMPLES + [dict(context=context, passage=passage, rewrite=stop(""))],
    )
    return "\n\n".join((PREFIX, "\n\n".join(examples)))
