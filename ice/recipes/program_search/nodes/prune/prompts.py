...
EXAMPLES: list[dict[str, str | ValueTransform | StopSentinel]] = [
    ...
    dict(
        ...
        resoning=RESONINGS[2],
    ),
]

INSTRUCTIONS = """For each question and list of excerpts, record which excerpts help answer the question, from most to least important."""
...
def make_pruning_prompt(
    *,
    ...
    all_examples = (examples or EXAMPLES[1:2]) + [
        dict(
            question=question,
            existing=numbered_list(existing),
            resoning=stop("- Excerpt 1"),
        )
    ]
...
def get_pruned_selections_via_logprobs(
    ...
    n_tokens = len(lps["tokens"])

    selections = [
        idx
        for idx in selections
        if idx > n_tokens - 20
    ] # only consider selections at the end of the prompt
...
def get_pruned_selections_via_completion(completion: str) -> Sequence[int]:
    print(completion)
    completion = completion.split("Which excerpts answer the question, from most to least important:")[1].strip()
