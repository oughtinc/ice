...
def remaining_prompts(
    ...
    return list(
        set(prompts_and_completions) - set(used_prompts_and_completions)
    )
...
async def eval_prompt(
    ...
    prompts = [few_shot_prompt+split_string+p for p, completion in test_prompts_and_completions]
...
async def best_few_shot(
    ...
    for prompt_perm in permutations(prompts_and_completions, n):
        ...
        remaining_prompts_and_completions = remaining_prompts(prompts_and_completions, list(prompt_perm))
        all_prompts.append((prompt, remaining_prompts_and_completions))

    scored_prompts = [
        (prompt, await eval_prompt(prompt, remaining_prompts_and_completions, split_string))
