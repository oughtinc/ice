...
class SummarizeUntilBest(Recipe):
    async def run(
        ...
        for cycle in range(1, max_cycles + 1):
            current_summaries = SummarizeNTimes.run(passage, background=background, attempts=attempts_per_cycle)
            if best_summary is None:
                grouped_summaries = '\n'.join(current_summaries)
                best_summary =  RankParagraphs(grouped_summaries, '', 1, prompt_type='summary')[0]
            else:
                grouped_summaries = '\n'.join([best_summary] + current_summaries)
                new_best_summary = RankParagraphs(grouped_summaries, '', 1, prompt_type='summary')[0]
