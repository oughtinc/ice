...
async def ask_subquestions(
    question: str = "What is the effect of creatine on cognition?", engine: str = "gpt-4"
):
    prompt = make_subquestion_prompt(question)
    subquestions_text = await recipe.agent(agent_name=engine).complete(prompt=prompt)
